...
const VehiclesRateList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`https://qbus.onrender.com/api/v1/get-all-rates-per-km-list`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`https://qbus.onrender.com/api/v1/admin/delete-particular-rate-per-km`, {
        headers: {
